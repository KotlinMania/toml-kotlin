# Claude Code Project Instructions — toml-kotlin

## Project Overview

This is **toml-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`toml`](https://crates.io/crates/toml).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/toml/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/toml>
